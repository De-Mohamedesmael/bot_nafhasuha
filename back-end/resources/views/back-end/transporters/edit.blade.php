...
@section('title', __('lang.edit_transporter'))
@section('styles')
    <style>
        .crop-btn,.delete-btn {
            position: absolute;
                ...
                <div class="col-md-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            {!! Form::open(['url' => route('admin.transporters.update', $transporter->id), 'id' =>
                            'transporter-form',
                            'method' =>
                            ...
                            <div class="row">
                                ...
                                <button class="btn btn-default bg-white btn-flat translation_btn" type="button"
                                    ...
                                    @include('back-end.layouts.partials.translation_inputs', [
                                        'attribute' => 'title',
                                        'translations' => $transporter->getTranslationsArray('title'),
                                        'type' => 'notifications',
                                    ])
                                </div>
                                            ...
                                            <div class="col-10 offset-1">
                                                ...
                                                <div class="preview-edit-container">
                                                    @if($transporter->image)
                                                        <div id="preview{{ $transporter->id }}" class="preview">
                                                            <input type="hidden"  name="have_image"  value="1" id="have-image">
                                                            <img
                                                                src="{{  asset('assets/images/'.$transporter->image) }}"
                                                                id="img{{  $transporter->id }}" alt="">
                                                            <div class="action_div"></div>
                                                            <button type="button"
                                                                    class="delete-btn"><i
                                                                    style="font-size: 20px;"
                                                                    data-href="{{ route('admin.transporters.deleteImage', $transporter->id) }}"
                                                                    id="deleteBtn{{ $transporter->id }}"
...
@section('javascript')
    <script>
        ...
        $("#submit-btn").on("click", function (e) {
            ...
            setTimeout(() => {
                if ($("#transporter-form").valid()) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: $("#transporter-form").attr("action"),
                        data: $("#transporter-form").serialize(),
        ...
        @if($transporter)
        {{--document.getElementById("cropBtn{{ $transporter->id }}").addEventListener('click', () => {--}}
        {{--    setTimeout(() => {--}}
        {{--        launchEditProductCropTool(document.getElementById("img{{ $transporter->id }}"));--}}
        {{--    }, 500);--}}
        {{--});--}}
        document.getElementById("deleteBtn{{ $transporter->id }}").addEventListener('click', () => {
            Swal.fire({
