...
@section('title', __('lang.add_notification'))
@section('styles')
    <style>
...
@endsection
@section('sli_li')
    <span class="parent"> < <a href="{{route('admin.notifications.index')}}">{{__('lang.notifications')}}</a>  / </span>  @lang('lang.add_notification')
@endsection
@section('content')
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header d-flex align-items-center">
                            <div class="print-title">@lang('lang.add_notification')</div>
                        </div>
                        <div class="card-body">
                            <p class="italic"><small>@lang('lang.required_fields_info')</small></p>
                            {!! Form::open(['url' => route('admin.notifications.store'), 'id' => 'notifications-form', 'method' => 'POST', 'class' => '', 'enctype' => 'multipart/form-data']) !!}

                            @include('back-end.notifications.partial.create_form')
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $('#type_model').on('change', function(){
            ...
            $('.div_type_model_id').hide();
            $('.div_type_model_id').removeClass('hide');
            $('#div_'+type).show();
            $('.input_User').attr('required',false);
            $('.input_Provider').attr('required',false);
            $('.input_'+type).attr('required',true);
        ...
        function submitForm() {
            if ($("#notifications-form").valid()) {
                tinyMCE.triggerSave();
                document.getElementById("loader").style.display = "block";
                document.getElementById("content").style.display = "none";
                $.ajax({
                    type: "POST",
                    url: $("form#notifications-form").attr("action"),
