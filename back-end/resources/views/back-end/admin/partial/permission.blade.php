                ...
                @if (!empty($subModulePermissionArray[$key_module]))
                    ...
                    @foreach ($sub_module_permission_array as $key_sub_module => $sub_module)
                        <tr class="sub_module_permission_{{ $key_module }}">
                            ...
                            <td>{{ $sub_module }}</td>
                            @php
                                $specialModulePermissionArray=\App\Models\Admin::specialModulePermissionArray();
                            @endphp
                            <td class="">
                                {!! Form::checkbox('special['. $key_module . '.' . $key_sub_module . '.special'.']',$key_sub_module, in_array($key_sub_module,$specialModulePermissionArray) ? (!empty($admin) && !empty($admin->hasPermissionTo( $key_module . '.' . $key_sub_module . '.special')) ? true : false):false, ['class' => 'checked_all', 'title' => __('lang.select_all')]) !!}
                            </td>
                            @php
                            ...
                            @if (Spatie\Permission\Models\Permission::where('name', $view_permission)->first())
                                <td class="">
                                    {!! Form::checkbox('permissions[' . $view_permission . ']', 1, !empty($admin) && !empty($admin->hasPermissionTo($view_permission)) ? true : false, ['class' => 'check_box check_box_view', 'title' => __('lang.view')]) !!}
                                </td>
                            @endif
                            @if (Spatie\Permission\Models\Permission::where('name', $create_permission)->first())
                                <td class="">
                                    {!! Form::checkbox('permissions[' . $create_permission . ']', 1, !empty($admin) && !empty($admin->hasPermissionTo($create_permission)) ? true : false, ['class' => 'check_box check_box_create', 'title' => __('lang.create')]) !!}
                                </td>
                            @endif
                            @if (Spatie\Permission\Models\Permission::where('name', $edit_permission)->first())
                                <td class="">
                                    {!! Form::checkbox('permissions[' . $edit_permission . ']', 1, !empty($admin) && !empty($admin->hasPermissionTo($edit_permission)) ? true : false, ['class' => 'check_box check_box_edit', 'title' => __('lang.edit')]) !!}
                                </td>
                            @endif
                            @if (Spatie\Permission\Models\Permission::where('name', $delete_permission)->first())
                                <td class="">
                                        {!! Form::checkbox('permissions[' . $delete_permission . ']', 1, !empty($admin) && !empty($admin->hasPermissionTo($delete_permission)) ? true : false, ['class' => 'check_box check_box_delete', 'title' => __('lang.delete')]) !!}
                                </td>
                            @endif
                        </tr>
                    @endforeach
